...
export const RawMaterialIssue = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v3_3_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v3_4_history')) || [];
    ...
    if (!master || master.length === 0) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v3_3_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v3_4_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { issueId: 'RMI-2026-' + Math.floor(Math.random()*10000) };
    ...
    const readOnlyFields = ["batchNo","productionOrderNo","issueId","sourceStockId","paddyVariety","lotNo"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v3_4_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v3_4_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Batch No","accessor":"batchNo"},{"header":"Lot No","accessor":"lotNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Paddy Variety","accessor":"paddyVariety"},{"header":"Qty to Process MT","accessor":"qtyToProcess"},{"header":"Source Stock ID","accessor":"sourceStockId"},{"header":"Source Warehouse","accessor":"sourceWarehouse"},{"header":"Operator","accessor":"operatorAllocated"}];
  const historyCols = [{"header":"Issue ID","accessor":"issueId"},{"header":"Batch No","accessor":"batchNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Issue Date","accessor":"issueDate"},{"header":"Paddy Stock ID","accessor":"sourceStockId"},{"header":"Paddy Qty Issued MT","accessor":"paddyQtyIssued"},{"header":"PP Bags Issued","accessor":"ppBagsIssued"},{"header":"BOPP Bags Issued","accessor":"boppBagsIssued"},{"header":"Thread Issued","accessor":"threadIssued"},{"header":"Labels Issued","accessor":"labelsIssued"},{"header":"Issued By","accessor":"issuedBy"},{"header":"Received By","accessor":"receivedBy"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 4 - Raw Material Issue</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 4 - Raw Material Issue Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Issue Date & Time</Label>
              <Input 
                type="datetime-local"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Paddy Stock ID</Label>
              <Input 
                type="text"
                value={formData.sourceStockId || ''} 
                onChange={(e) => setFormData({...formData, sourceStockId: e.target.value})}
                readOnly={true}
                className={'w-full mt-1.5 bg-slate-50 border-slate-200 text-slate-600 cursor-not-allowed rounded-md'}
              />
            </div>
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Paddy Quantity Issued MT</Label>
              <Input 
                type="number"
                value={formData.paddyQtyIssued || ''} 
                onChange={(e) => setFormData({...formData, paddyQtyIssued: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">PP Bags — Item Code & Qty Issued (Pcs)</Label>
              <Input 
                type="text"
                value={formData.ppBagsIssued || ''} 
                onChange={(e) => setFormData({...formData, ppBagsIssued: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">BOPP Bags — Item Code & Qty Issued (Pcs)</Label>
              <Input 
                type="text"
                value={formData.boppBagsIssued || ''} 
                onChange={(e) => setFormData({...formData, boppBagsIssued: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Thread — Item Code & Qty Issued (Rolls)</Label>
              <Input 
                type="text"
                value={formData.threadIssued || ''} 
                onChange={(e) => setFormData({...formData, threadIssued: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Labels — Item Code & Qty Issued (Rolls)</Label>
              <Input 
                type="text"
                value={formData.labelsIssued || ''} 
                onChange={(e) => setFormData({...formData, labelsIssued: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Pallets — Qty Issued (Pcs)</Label>
              <Input 
                type="number"
                value={formData.palletsIssued || ''} 
                onChange={(e) => setFormData({...formData, palletsIssued: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Lubricants/Oil</Label>
              <Input 
                type="text"
                value={formData.lubricantsIssued || ''} 
                onChange={(e) => setFormData({...formData, lubricantsIssued: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Other Consumables</Label>
              <Input 
                type="text"
                value={formData.otherConsumables || ''} 
                onChange={(e) => setFormData({...formData, otherConsumables: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Issue Remarks</Label>
              <Input 
                type="text"
