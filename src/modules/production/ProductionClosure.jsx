...
export const ProductionClosure = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v4_16_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v4_17_history')) || [];
    ...
    if (!master || master.length === 0 || !master[0].productionOrderNo) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v4_16_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v4_17_history')) || [];
    }
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v4_17_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v4_17_history', JSON.stringify(rawHistory));
    }
