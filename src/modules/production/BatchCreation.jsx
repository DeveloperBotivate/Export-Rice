...
export const BatchCreation = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v3_2_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v3_3_history')) || [];
    ...
    if (!master || master.length === 0) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v3_2_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v3_3_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { batchNo: 'BT-2026-' + Math.floor(Math.random()*10000), lotNo: 'LT-2026-' + Math.floor(Math.random()*10000) };
    ...
    const readOnlyFields = ["productionOrderNo","planNo","batchNo","lotNo","paddyVariety"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v3_3_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v3_3_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Plan No","accessor":"planNo"},{"header":"Paddy Variety","accessor":"paddyVariety"},{"header":"Planned Input MT","accessor":"plannedInputQty"},{"header":"Plant","accessor":"plant"},{"header":"Shift","accessor":"shift"},{"header":"Supervisor","accessor":"supervisor"},{"header":"Operator","accessor":"operatorAllocated"},{"header":"Start Date","accessor":"productionStartDate"}];
  const historyCols = [{"header":"Batch No","accessor":"batchNo"},{"header":"Lot No","accessor":"lotNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Batch Date","accessor":"batchDate"},{"header":"Paddy Variety","accessor":"paddyVariety"},{"header":"Source Stock ID","accessor":"sourceStockId"},{"header":"Source Warehouse","accessor":"sourceWarehouse"},{"header":"Qty to Process MT","accessor":"qtyToProcess"},{"header":"Machine Used","accessor":"machineUsed"},{"header":"Operator","accessor":"operatorAllocated"},{"header":"Batch Start Time","accessor":"batchStartTime"},{"header":"Batch End Time","accessor":"batchEndTime"},{"header":"Status","accessor":"status"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 3 - Batch Creation</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 3 - Batch Creation Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Batch Date</Label>
              <Input 
                type="date"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Paddy Variety / Grade</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Source Stock ID</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Source Warehouse / Go-down / Rack-Bin</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Quantity to Process MT (this batch)</Label>
              <Input 
                type="number"
                value={formData.qtyToProcess || ''} 
                onChange={(e) => setFormData({...formData, qtyToProcess: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Machine Used (Machine ID)</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Operator (Name/ID)</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Batch Start Time</Label>
              <Input 
                type="time"
                value={formData.batchStartTime || ''} 
                onChange={(e) => setFormData({...formData, batchStartTime: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Batch End Time (estimated)</Label>
              <Input 
                type="time"
                value={formData.batchEndTime || ''} 
                onChange={(e) => setFormData({...formData, batchEndTime: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Batch Remarks</Label>
              <Input 
                type="text"
                value={formData.batchRemarks || ''} 
                onChange={(e) => setFormData({...formData, batchRemarks: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
