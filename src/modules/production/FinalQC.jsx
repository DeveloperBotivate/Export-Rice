...
export const FinalQC = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v3_11_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v3_12_history')) || [];
    ...
    if (!master || master.length === 0) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v3_11_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v3_12_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { finalQcId: 'FQC-2026-' + Math.floor(Math.random()*10000) };
    ...
    const readOnlyFields = ["millingSessionId","batchNo","productionOrderNo","finalQcId"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v3_12_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v3_12_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Milling Session ID","accessor":"millingSessionId"},{"header":"Batch No","accessor":"batchNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Head Rice MT","accessor":"headRiceMt"},{"header":"Broken Rice MT","accessor":"brokenRiceMt"},{"header":"Husk MT","accessor":"huskMt"},{"header":"Bran MT","accessor":"branMt"},{"header":"Rejection MT","accessor":"rejectionMt"},{"header":"Power kWh","accessor":"powerConsumed"}];
  const historyCols = [{"header":"Final QC ID","accessor":"finalQcId"},{"header":"Milling Session ID","accessor":"millingSessionId"},{"header":"Batch No","accessor":"batchNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Inspection Date","accessor":"inspectionDate"},{"header":"Moisture %","accessor":"finalMoisturePercent"},{"header":"Broken %","accessor":"finalBrokenPercent"},{"header":"Chalky %","accessor":"finalChalkyPercent"},{"header":"Foreign Matter %","accessor":"finalForeignMatterPercent"},{"header":"Colour","accessor":"finalColour"},{"header":"Head Rice Grade","accessor":"finalHeadRiceGrade"},{"header":"Broken Rice Grade","accessor":"finalBrokenRiceGrade"},{"header":"QC Result","accessor":"finalQcResult"},{"header":"QC Inspector","accessor":"finalQcInspector"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 7 - Final Quality Inspection</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 7 - Final Quality Inspection Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Milling Session ID</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Inspection Date & Time</Label>
              <Input 
                type="datetime-local"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">QC Inspector</Label>
              <Input 
                type="text"
                value={formData.finalQcInspector || ''} 
                onChange={(e) => setFormData({...formData, finalQcInspector: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Moisture %</Label>
              <Input 
                type="number"
                value={formData.finalMoisturePercent || ''} 
                onChange={(e) => setFormData({...formData, finalMoisturePercent: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Broken Grain %</Label>
              <Input 
                type="number"
                value={formData.finalBrokenPercent || ''} 
                onChange={(e) => setFormData({...formData, finalBrokenPercent: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Chalky %</Label>
              <Input 
                type="number"
                value={formData.finalChalkyPercent || ''} 
                onChange={(e) => setFormData({...formData, finalChalkyPercent: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Foreign Matter %</Label>
              <Input 
                type="number"
                value={formData.finalForeignMatterPercent || ''} 
                onChange={(e) => setFormData({...formData, finalForeignMatterPercent: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Colour (L-Value / Visual)</Label>
              <Input 
                type="text"
                value={formData.finalColour || ''} 
                onChange={(e) => setFormData({...formData, finalColour: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Whiteness Degree</Label>
              <Input 
                type="number"
                value={formData.whitenessDegree || ''} 
                onChange={(e) => setFormData({...formData, whitenessDegree: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              >
                ...
                <option value="Premium">Premium</option><option value="Export">Export</option><option value="Domestic">Domestic</option><option value="Standard">Standard</option>
              </Select>
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Broken Rice Grade</Label>
              <Select 
                value={formData.finalBrokenRiceGrade || ''} 
                onChange={(e) => setFormData({...formData, finalBrokenRiceGrade: e.target.value})}
                disabled={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              >
                <option value="">Select...</option>
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">QC Result</Label>
              <Select 
                value={formData.finalQcResult || ''} 
                onChange={(e) => setFormData({...formData, finalQcResult: e.target.value})}
                disabled={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              >
                <option value="">Select...</option>
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Remarks for Dispatch / Packing</Label>
              <Input 
                type="text"
                value={formData.finalQcRemarks || ''} 
                onChange={(e) => setFormData({...formData, finalQcRemarks: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Inspector Signature Ref</Label>
              <Input 
                type="text"
