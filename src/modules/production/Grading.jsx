...
export const Grading = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v3_9_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v3_10_history')) || [];
    ...
    if (!master || master.length === 0) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v3_9_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v3_10_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { gradingId: 'GRD-2026-' + Math.floor(Math.random()*10000) };
    ...
    const readOnlyFields = ["polishingId","productionOrderNo","gradingId"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v3_10_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v3_10_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Polishing ID","accessor":"polishingId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"White Rice MT","accessor":"whiteRiceMt"},{"header":"Date","accessor":"millingDate"},{"header":"Operator","accessor":"operatorAllocated"}];
  const historyCols = [{"header":"Grading ID","accessor":"gradingId"},{"header":"Polishing ID","accessor":"polishingId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Input MT","accessor":"gradingInputMt"},{"header":"Grade A MT","accessor":"headRiceMt"},{"header":"Grade B MT","accessor":"gradeBRiceMt"},{"header":"Broken Rice MT","accessor":"brokenRiceMt"},{"header":"Small Broken MT","accessor":"smallBrokenMt"},{"header":"Date","accessor":"millingDate"},{"header":"Operator","accessor":"operatorAllocated"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 6.5 - Grading</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 6.5 - Grading Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Input - White Rice MT</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Grade A Rice MT</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Grade B Rice MT</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Small Broken MT</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Grading Machine ID</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Sieve Size Used</Label>
              <Input 
                type="text"
                value={formData.sieveSize || ''} 
                onChange={(e) => setFormData({...formData, sieveSize: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
