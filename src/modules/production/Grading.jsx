...
export const Grading = () => {
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { gradingId: 'GRD-' + Math.floor(Math.random()*10000) };
    
    
    const readOnlyFields = ["polishingId","productionOrderNo"];
  ...
  const pendingCols = [{"header":"Polishing ID","accessor":"polishingId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"White Rice Output (MT)","accessor":"whiteRiceOutput"},{"header":"Date","accessor":"polishingDate"},{"header":"Operator","accessor":"polishingOperator"}];
  const historyCols = [{"header":"Grading ID","accessor":"gradingId"},{"header":"Polishing ID","accessor":"polishingId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Input (MT)","accessor":"gradingInputQty"},{"header":"Grade A (MT)","accessor":"gradeARice"},{"header":"Grade B (MT)","accessor":"gradeBRice"},{"header":"Broken Rice (MT)","accessor":"brokenRice"},{"header":"Small Broken (MT)","accessor":"smallBroken"},{"header":"Date","accessor":"gradingDate"},{"header":"Operator","accessor":"gradingOperator"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 2.5 - Grading</h2>
      </div>
      ...
      >
        <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label>Input — White Rice (MT)</Label>
              <Input 
                type="number"
                value={formData.gradingInputQty || ''} 
                onChange={(e) => setFormData({...formData, gradingInputQty: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Grade A Rice (MT)</Label>
              <Input 
                type="number"
                value={formData.gradeARice || ''} 
                onChange={(e) => setFormData({...formData, gradeARice: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Grade B Rice (MT)</Label>
              <Input 
                type="number"
                value={formData.gradeBRice || ''} 
                onChange={(e) => setFormData({...formData, gradeBRice: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Broken Rice (MT)</Label>
              <Input 
                type="number"
                value={formData.brokenRice || ''} 
                onChange={(e) => setFormData({...formData, brokenRice: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Small Broken (MT)</Label>
              <Input 
                type="number"
                value={formData.smallBroken || ''} 
                onChange={(e) => setFormData({...formData, smallBroken: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.gradingMachineId || ''} 
                onChange={(e) => setFormData({...formData, gradingMachineId: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Sieve Size Used</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.gradingOperator || ''} 
                onChange={(e) => setFormData({...formData, gradingOperator: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                ...
                onChange={(e) => setFormData({...formData, gradingDate: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
