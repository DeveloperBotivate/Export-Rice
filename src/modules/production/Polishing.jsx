...
export const Polishing = () => {
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { polishingId: 'POL-' + Math.floor(Math.random()*10000) };
    
    
    
    const readOnlyFields = ["separationId","productionOrderNo"];
  ...
  const pendingCols = [{"header":"Separation ID","accessor":"separationId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Separated Brown Rice (MT)","accessor":"separatedBrownRice"},{"header":"Date","accessor":"separationDate"},{"header":"Operator","accessor":"separationOperator"}];
  const historyCols = [{"header":"Polishing ID","accessor":"polishingId"},{"header":"Separation ID","accessor":"separationId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Input (MT)","accessor":"polishingInputQty"},{"header":"White Rice Output (MT)","accessor":"whiteRiceOutput"},{"header":"Bran Output (MT)","accessor":"branOutput"},{"header":"Polish Output (MT)","accessor":"polishOutput"},{"header":"Polishing Degree %","accessor":"polishingDegree"},{"header":"Date","accessor":"polishingDate"},{"header":"Operator","accessor":"polishingOperator"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 2.4 - Polishing</h2>
      </div>
      ...
      >
        <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label>Input — Brown Rice (MT)</Label>
              <Input 
                type="number"
                value={formData.polishingInputQty || ''} 
                onChange={(e) => setFormData({...formData, polishingInputQty: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>White Rice Output (MT)</Label>
              <Input 
                type="number"
                value={formData.whiteRiceOutput || ''} 
                onChange={(e) => setFormData({...formData, whiteRiceOutput: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Rice Bran Generated (MT)</Label>
              <Input 
                type="number"
                value={formData.branOutput || ''} 
                onChange={(e) => setFormData({...formData, branOutput: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Rice Polish Generated (MT)</Label>
              <Input 
                type="number"
                value={formData.polishOutput || ''} 
                onChange={(e) => setFormData({...formData, polishOutput: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Polishing Degree (%)</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.polishingMachineId || ''} 
                onChange={(e) => setFormData({...formData, polishingMachineId: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                ...
                onChange={(e) => setFormData({...formData, waterUsed: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.polishingOperator || ''} 
                onChange={(e) => setFormData({...formData, polishingOperator: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                type="datetime-local"
                value={formData.polishingDate || ''} 
                onChange={(e) => setFormData({...formData, polishingDate: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
