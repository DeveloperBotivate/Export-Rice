...
export const PaddyCleaning = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v3_5_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v3_6_history')) || [];
    ...
    if (!master || master.length === 0) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v3_5_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v3_6_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { cleaningId: 'CLN-2026-' + Math.floor(Math.random()*10000) };
    ...
    const readOnlyFields = ["productionOrderNo","cleaningId"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v3_6_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v3_6_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Paddy Grade","accessor":"paddyGradeConfirmed"},{"header":"Planned Input MT","accessor":"plannedInputQty"},{"header":"Start Date","accessor":"productionStartDate"},{"header":"Machine ID","accessor":"machineAllocated"},{"header":"Operator","accessor":"operatorAllocated"}];
  const historyCols = [{"header":"Cleaning ID","accessor":"cleaningId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Input Qty MT","accessor":"cleaningInputMt"},{"header":"Cleaned Output MT","accessor":"cleanedOutputMt"},{"header":"Dust Wastage Kg","accessor":"dustRemovedKg"},{"header":"Stone Wastage Kg","accessor":"stonesRemovedKg"},{"header":"Foreign Matter Kg","accessor":"foreignMatterRemovedKg"},{"header":"Date","accessor":"millingDate"},{"header":"Operator","accessor":"operatorAllocated"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 6.1 - Paddy Cleaning</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 6.1 - Paddy Cleaning Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Cleaned Output MT</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Wastage - Dust Kg</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Wastage - Stones Kg</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Foreign Matter Removed Kg</Label>
              <Input 
                type="number"
                value={formData.foreignMatterRemovedKg || ''} 
                onChange={(e) => setFormData({...formData, foreignMatterRemovedKg: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
