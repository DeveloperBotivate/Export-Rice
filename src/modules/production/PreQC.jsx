...
export const PreQC = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v3_4_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v3_5_history')) || [];
    ...
    if (!master || master.length === 0) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v3_4_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v3_5_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { preQcId: 'PQC-2026-' + Math.floor(Math.random()*10000) };
    ...
    const readOnlyFields = ["issueId","batchNo","productionOrderNo","preQcId"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v3_5_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v3_5_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Issue ID","accessor":"issueId"},{"header":"Batch No","accessor":"batchNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Paddy Variety","accessor":"paddyVariety"},{"header":"Paddy Qty Issued MT","accessor":"paddyQtyIssued"},{"header":"Lot No","accessor":"lotNo"},{"header":"Issue Date","accessor":"issueDate"},{"header":"Issued By","accessor":"issuedBy"}];
  const historyCols = [{"header":"Pre-QC ID","accessor":"preQcId"},{"header":"Issue ID","accessor":"issueId"},{"header":"Batch No","accessor":"batchNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Test Date","accessor":"testDate"},{"header":"Moisture %","accessor":"moisturePercent"},{"header":"Foreign Matter %","accessor":"foreignMatterPercent"},{"header":"Broken %","accessor":"brokenPercent"},{"header":"Chalky %","accessor":"chalkyPercent"},{"header":"Paddy Grade Confirmed","accessor":"paddyGradeConfirmed"},{"header":"Expected Recovery %","accessor":"expectedRecoveryAfterTest"},{"header":"QC Result","accessor":"qcResult"},{"header":"Approved for Production","accessor":"approvedForProduction"},{"header":"Lab Technician","accessor":"labTechnician"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 5 - Pre-Production QC</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 5 - Pre-Production QC Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Issue ID</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Pre-QC ID</Label>
              <Input 
                type="text"
                value={formData.preQcId || ''} 
                onChange={(e) => setFormData({...formData, preQcId: e.target.value})}
                readOnly={true}
                className={'w-full mt-1.5 bg-slate-50 border-slate-200 text-slate-600 cursor-not-allowed rounded-md'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Test Date & Time</Label>
              <Input 
                type="datetime-local"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Lab Technician</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              ...
              <Input 
                type="number"
                value={formData.foreignMatterPercent || ''} 
                onChange={(e) => setFormData({...formData, foreignMatterPercent: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Immature Grain %</Label>
              <Input 
                type="number"
                value={formData.immatureGrainPercent || ''} 
                onChange={(e) => setFormData({...formData, immatureGrainPercent: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Damaged/Discoloured Grain %</Label>
              <Input 
                type="number"
                value={formData.damagedGrainPercent || ''} 
                onChange={(e) => setFormData({...formData, damagedGrainPercent: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Chalky Grain %</Label>
              <Input 
                type="number"
                value={formData.chalkyPercent || ''} 
                onChange={(e) => setFormData({...formData, chalkyPercent: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Red Grain %</Label>
              <Input 
                type="number"
                value={formData.redGrainPercent || ''} 
                onChange={(e) => setFormData({...formData, redGrainPercent: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Paddy Grade Confirmed</Label>
              <Select 
                value={formData.paddyGradeConfirmed || ''} 
                onChange={(e) => setFormData({...formData, paddyGradeConfirmed: e.target.value})}
                disabled={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              >
            ...
            <div className="space-y-1">
              ...
              <Input 
                type="number"
                value={formData.expectedRecoveryAfterTest || ''} 
                onChange={(e) => setFormData({...formData, expectedRecoveryAfterTest: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Approval for Production</Label>
              <Select 
                value={formData.approvedForProduction || ''} 
                onChange={(e) => setFormData({...formData, approvedForProduction: e.target.value})}
                disabled={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              >
                <option value="">Select...</option>
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Rejection Reason (if Fail)</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">QC Remarks</Label>
              <Input 
                type="text"
