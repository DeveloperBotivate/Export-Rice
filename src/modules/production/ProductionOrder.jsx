...
export const ProductionOrder = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v4_1_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v4_2_history')) || [];
    ...
    if (!master || master.length === 0 || !master[0].productionOrderNo) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v4_1_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v4_2_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    let autoFields = {};
    autoFields = { productionOrderNo: 'PRD-2026-' + Math.floor(Math.random()*10000) };
    ...
    const readOnlyFields = ["planNo","productionOrderNo","paddyVariety","plannedInputQty","plant","shift","supervisor","machineAllocated"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v4_2_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v4_2_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Plan No","accessor":"planNo"},{"header":"Plan Date","accessor":"planDate"},{"header":"Paddy Variety","accessor":"paddyVariety"},{"header":"Planned Input MT","accessor":"plannedInputQty"},{"header":"Target Head Rice MT","accessor":"targetHeadRice"},{"header":"Expected Recovery %","accessor":"expectedRecovery"},{"header":"Plant","accessor":"plant"},{"header":"Shift","accessor":"shift"},{"header":"Supervisor","accessor":"supervisor"},{"header":"Planned By","accessor":"plannedBy"}];
  const historyCols = [{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Plan No","accessor":"planNo"},{"header":"Order Date","accessor":"orderDate"},{"header":"Paddy Variety","accessor":"paddyVariety"},{"header":"Planned Input MT","accessor":"plannedInputQty"},{"header":"Plant","accessor":"plant"},{"header":"Shift","accessor":"shift"},{"header":"Supervisor","accessor":"supervisor"},{"header":"Operator","accessor":"operatorAllocated"},{"header":"Start Date","accessor":"productionStartDate"},{"header":"End Date","accessor":"productionEndDate"},{"header":"Priority","accessor":"priority"},{"header":"Status","accessor":"status"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 2 - Production Order</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 2 - Production Order Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Planned Input Quantity MT</Label>
              <Input 
                type="number"
                value={formData.plannedInputQty || ''} 
                onChange={(e) => setFormData({...formData, plannedInputQty: e.target.value})}
                readOnly={true}
                className={'w-full mt-1.5 bg-slate-50 border-slate-200 text-slate-600 cursor-not-allowed rounded-md'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Plant / Mill Unit</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Production Start Date</Label>
              <Input 
                type="date"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Production End Date (estimated)</Label>
              <Input 
                type="date"
            ...
            <div className="space-y-1">
              ...
              >
                <option value="">Select...</option>
                <option value="Open">Open</option><option value="In Progress">In Progress</option><option value="Completed">Completed</option><option value="On Hold">On Hold</option>
              </Select>
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Order Remarks</Label>
              <Input 
                type="text"
