...
export const ExpectedVsActual = () => {
  ...
  const handleActionClick = (item) => {
    ...
    const readOnlyFields = ["productionOrderNo","colorSortId","actualRecoveryPercent","recoveryTarget","variance"];
  ...
  const pendingCols = [{"header":"Color Sort ID","accessor":"colorSortId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Planned Input (MT)","accessor":"plannedInputQty"},{"header":"Expected Rice Output (MT)","accessor":"expectedPremiumRice"},{"header":"Sorted Output (MT)","accessor":"sortedOutput"},{"header":"Rejection (MT)","accessor":"rejection"},{"header":"Date","accessor":"colorSortDate"}];
  const historyCols = [{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Planned Input (MT)","accessor":"plannedInputQty"},{"header":"Actual Input (MT)","accessor":"actualTotalInput"},{"header":"Expected Output (MT)","accessor":"expectedPremiumRice"},{"header":"Actual Output (MT)","accessor":"actualRiceOutput"},{"header":"Variance (MT)","accessor":"variance"},{"header":"Recovery %","accessor":"actualRecoveryPercent"},{"header":"Actual Bran (MT)","accessor":"actualBran"},{"header":"Actual Husk (MT)","accessor":"actualHusk"},{"header":"Machine Hold","accessor":"machineHold"},{"header":"Closure Date","accessor":"closureDate"},{"header":"Closed By","accessor":"closedBy"},{"header":"Status","accessor":"status"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 3 - Expected vs Actual (Inventory)</h2>
      </div>
      ...
      >
        <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label>Actual Total Input (MT)</Label>
              <Input 
                type="number"
                value={formData.actualTotalInput || ''} 
                onChange={(e) => setFormData({...formData, actualTotalInput: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Actual Rice Output (MT)</Label>
              <Input 
                type="number"
                value={formData.actualRiceOutput || ''} 
                onChange={(e) => setFormData({...formData, actualRiceOutput: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Actual Bran (MT)</Label>
              <Input 
                type="number"
                value={formData.actualBran || ''} 
                onChange={(e) => setFormData({...formData, actualBran: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Actual Husk (MT)</Label>
              <Input 
                type="number"
                value={formData.actualHusk || ''} 
                onChange={(e) => setFormData({...formData, actualHusk: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Actual Polish (MT)</Label>
              <Input 
                type="number"
                value={formData.actualPolish || ''} 
                onChange={(e) => setFormData({...formData, actualPolish: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Actual Broken Rice (MT)</Label>
              <Input 
                type="number"
                value={formData.actualBrokenRice || ''} 
                onChange={(e) => setFormData({...formData, actualBrokenRice: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                type="number"
                value={formData.actualRecoveryPercent || ''} 
                onChange={(e) => setFormData({...formData, actualRecoveryPercent: e.target.value})}
                readOnly={true}
                className={true ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                type="number"
                value={formData.recoveryTarget || ''} 
                onChange={(e) => setFormData({...formData, recoveryTarget: e.target.value})}
                readOnly={true}
                className={true ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Variance (MT)</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1.5">
              <Label>Machine Hold (Yes/No)</Label>
              <Select 
                value={formData.machineHold || ''} 
                onChange={(e) => setFormData({...formData, machineHold: e.target.value})}
                disabled={false}
                className={false ? 'bg-slate-100' : ''}
              >
                <option value="">Select Machine Hold (Yes/No)</option>
            ...
            <div className="space-y-1.5">
              <Label>Inventory Transfer Ref</Label>
              <Input 
                type="text"
                value={formData.inventoryTransferRef || ''} 
                onChange={(e) => setFormData({...formData, inventoryTransferRef: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
