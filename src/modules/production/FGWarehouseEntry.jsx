...
export const FGWarehouseEntry = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v3_15_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v3_16_history')) || [];
    ...
    if (!master || master.length === 0) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v3_15_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v3_16_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { fgwEntryId: 'FGW-2026-' + Math.floor(Math.random()*10000) };
    
    
    const readOnlyFields = ["packingOrderNo","batchNo","productionOrderNo","fgwEntryId","finalHeadRiceGrade","headRiceLotNo","noOfBagsRequired","totalPackedMt","bestBeforeDate","fgwStockValue"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v3_16_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v3_16_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Packing Order No","accessor":"packingOrderNo"},{"header":"Batch No","accessor":"batchNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Rice Grade","accessor":"finalHeadRiceGrade"},{"header":"Lot No","accessor":"headRiceLotNo"},{"header":"No. of Bags","accessor":"noOfBagsRequired"},{"header":"Total Packed MT","accessor":"totalPackedMt"},{"header":"Packing QC Result","accessor":"packingQcResult"},{"header":"Packing Date","accessor":"packingDate"}];
  const historyCols = [{"header":"FG Warehouse Entry ID","accessor":"fgwEntryId"},{"header":"Packing Order No","accessor":"packingOrderNo"},{"header":"Batch No","accessor":"batchNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Entry Date","accessor":"fgwEntryDate"},{"header":"Rice Grade","accessor":"finalHeadRiceGrade"},{"header":"Lot No","accessor":"headRiceLotNo"},{"header":"No. of Bags","accessor":"noOfBagsRequired"},{"header":"Total Net Weight MT","accessor":"totalPackedMt"},{"header":"FG Warehouse","accessor":"fgWarehouse"},{"header":"Go-down","accessor":"fgGodown"},{"header":"Rack","accessor":"fgRack"},{"header":"Bin","accessor":"fgBin"},{"header":"FEFO Date","accessor":"bestBeforeDate"},{"header":"Stock Value ₹","accessor":"fgwStockValue"},{"header":"Entry Done By","accessor":"fgwEntryDoneBy"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 11 - FG Warehouse Entry</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 11 - FG Warehouse Entry Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Entry Date & Time</Label>
              <Input 
                type="datetime-local"
                value={formData.fgwEntryDate || ''} 
                onChange={(e) => setFormData({...formData, fgwEntryDate: e.target.value})}
                readOnly={false}
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Total Net Weight MT</Label>
              <Input 
                type="number"
                value={formData.totalPackedMt || ''} 
                onChange={(e) => setFormData({...formData, totalPackedMt: e.target.value})}
                readOnly={true}
                className={'w-full mt-1.5 bg-slate-50 border-slate-200 text-slate-600 cursor-not-allowed rounded-md'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">FG Warehouse</Label>
              <Input 
                type="text"
                value={formData.fgWarehouse || ''} 
                onChange={(e) => setFormData({...formData, fgWarehouse: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">FG Go-down</Label>
              <Input 
                type="text"
                value={formData.fgGodown || ''} 
                onChange={(e) => setFormData({...formData, fgGodown: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">FG Rack</Label>
              <Input 
                type="text"
                value={formData.fgRack || ''} 
                onChange={(e) => setFormData({...formData, fgRack: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">FG Bin</Label>
              <Input 
                type="text"
                value={formData.fgBin || ''} 
                onChange={(e) => setFormData({...formData, fgBin: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              ...
              >
                ...
                <option value="Ambient">Ambient</option><option value="Cool">Cool</option><option value="Dry">Dry</option>
              </Select>
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">FEFO Date (= Best Before)</Label>
              <Input 
                type="date"
            ...
            <div className="space-y-1">
              ...
              <Input 
                type="number"
                value={formData.fgwValuationRate || ''} 
                onChange={(e) => setFormData({...formData, fgwValuationRate: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Total Stock Value ₹</Label>
              <Input 
                type="number"
                value={formData.fgwStockValue || ''} 
                onChange={(e) => setFormData({...formData, fgwStockValue: e.target.value})}
                readOnly={true}
                className={'w-full mt-1.5 bg-slate-50 border-slate-200 text-slate-600 cursor-not-allowed rounded-md'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Entry Done By</Label>
              <Input 
                type="text"
                value={formData.fgwEntryDoneBy || ''} 
                onChange={(e) => setFormData({...formData, fgwEntryDoneBy: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Warehouse Incharge Signature Ref</Label>
              <Input 
                type="text"
                value={formData.warehouseInchargeSig || ''} 
                onChange={(e) => setFormData({...formData, warehouseInchargeSig: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              ...
              <Input 
                type="text"
                value={formData.fgwRemarks || ''} 
                onChange={(e) => setFormData({...formData, fgwRemarks: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
