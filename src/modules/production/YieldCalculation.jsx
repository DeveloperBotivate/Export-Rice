...
export const YieldCalculation = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v3_12_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v3_13_history')) || [];
    ...
    if (!master || master.length === 0) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v3_12_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v3_13_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { yieldId: 'YLD-2026-' + Math.floor(Math.random()*10000) };
    
    
    
    
    
    const readOnlyFields = ["finalQcId","batchNo","productionOrderNo","yieldId","headRiceRecovery","totalRiceRecovery","branRecovery","huskRecovery","millingPercent","varianceMt","variancePercent"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v3_13_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v3_13_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Final QC ID","accessor":"finalQcId"},{"header":"Milling Session ID","accessor":"millingSessionId"},{"header":"Batch No","accessor":"batchNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Head Rice Grade","accessor":"finalHeadRiceGrade"},{"header":"QC Result","accessor":"finalQcResult"},{"header":"Inspection Date","accessor":"inspectionDate"},{"header":"QC Inspector","accessor":"finalQcInspector"}];
  const historyCols = [{"header":"Yield ID","accessor":"yieldId"},{"header":"Batch No","accessor":"batchNo"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Yield Date","accessor":"yieldDate"},{"header":"Paddy Input MT","accessor":"totalPaddyInputMt"},{"header":"Head Rice MT","accessor":"headRiceMt"},{"header":"Broken Rice MT","accessor":"brokenRiceMt"},{"header":"Bran MT","accessor":"branMt"},{"header":"Husk MT","accessor":"huskMt"},{"header":"Polish MT","accessor":"ricePolishMt"},{"header":"Waste MT","accessor":"wasteMt"},{"header":"Head Rice Recovery %","accessor":"headRiceRecovery"},{"header":"Total Recovery %","accessor":"totalRiceRecovery"},{"header":"Milling %","accessor":"millingPercent"},{"header":"Expected Head Rice MT","accessor":"expectedHeadRiceMt"},{"header":"Variance MT","accessor":"varianceMt"},{"header":"Variance %","accessor":"variancePercent"},{"header":"Machine Hours","accessor":"totalMachineHours"},{"header":"Power kWh","accessor":"powerConsumed"},{"header":"Downtime Hours","accessor":"downtimeHours"},{"header":"Calculated By","accessor":"calculatedBy"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 8 - Yield & Recovery Calculation</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 8 - Yield & Recovery Calculation Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Final QC ID</Label>
              <Input 
                type="text"
                value={formData.finalQcId || ''} 
                onChange={(e) => setFormData({...formData, finalQcId: e.target.value})}
                readOnly={true}
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Yield Date</Label>
              <Input 
                type="date"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Dust/Waste/Rejection MT</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Bran Recovery %</Label>
              <Input 
                type="number"
                value={formData.branRecovery || ''} 
                onChange={(e) => setFormData({...formData, branRecovery: e.target.value})}
                readOnly={true}
                className={'w-full mt-1.5 bg-slate-50 border-slate-200 text-slate-600 cursor-not-allowed rounded-md'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Husk Recovery %</Label>
              <Input 
                type="number"
                value={formData.huskRecovery || ''} 
                onChange={(e) => setFormData({...formData, huskRecovery: e.target.value})}
                readOnly={true}
                className={'w-full mt-1.5 bg-slate-50 border-slate-200 text-slate-600 cursor-not-allowed rounded-md'}
              />
            </div>
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Expected Head Rice MT</Label>
              <Input 
                type="number"
                value={formData.expectedHeadRiceMt || ''} 
                onChange={(e) => setFormData({...formData, expectedHeadRiceMt: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Actual Head Rice MT</Label>
              <Input 
                type="number"
                value={formData.actualHeadRiceMt || ''} 
                onChange={(e) => setFormData({...formData, actualHeadRiceMt: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Variance MT</Label>
              <Input 
                type="number"
                value={formData.varianceMt || ''} 
                onChange={(e) => setFormData({...formData, varianceMt: e.target.value})}
                readOnly={true}
                className={'w-full mt-1.5 bg-slate-50 border-slate-200 text-slate-600 cursor-not-allowed rounded-md'}
              />
            </div>
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Power Consumed kWh</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Downtime Hours</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Downtime Reason</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Calculated By</Label>
              <Input 
                type="text"
