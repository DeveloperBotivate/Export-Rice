...
export const Dehusking = () => {
  const getInitialData = () => {
    let rawPending = JSON.parse(localStorage.getItem('prod_v4_6_history')) || [];
    let rawHistory = JSON.parse(localStorage.getItem('prod_v4_7_history')) || [];
    ...
    if (!master || master.length === 0 || !master[0].productionOrderNo) {
      ...
      rawPending = JSON.parse(localStorage.getItem('prod_v4_6_history')) || [];
      rawHistory = JSON.parse(localStorage.getItem('prod_v4_7_history')) || [];
    }
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { dehuskingId: 'DHK-2026-' + Math.floor(Math.random()*10000) };
    ...
    const readOnlyFields = ["cleaningId","productionOrderNo","dehuskingId"];
  ...
  const handleSave = () => {
    ...
    const rawHistory = JSON.parse(localStorage.getItem('prod_v4_7_history')) || [];
    if (!rawHistory.includes(processedItem.id)) {
      rawHistory.unshift(processedItem.id);
      localStorage.setItem('prod_v4_7_history', JSON.stringify(rawHistory));
    }
  ...
  const pendingCols = [{"header":"Cleaning ID","accessor":"cleaningId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Cleaned Output MT","accessor":"cleanedOutputMt"},{"header":"Date","accessor":"millingDate"},{"header":"Operator","accessor":"operatorAllocated"}];
  const historyCols = [{"header":"Dehusking ID","accessor":"dehuskingId"},{"header":"Cleaning ID","accessor":"cleaningId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Input Qty MT","accessor":"dehuskingInputMt"},{"header":"Brown Rice MT","accessor":"brownRiceMt"},{"header":"Husk MT","accessor":"huskMt"},{"header":"Efficiency %","accessor":"dehuskingEfficiency"},{"header":"Date","accessor":"millingDate"},{"header":"Operator","accessor":"operatorAllocated"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 6.2 - Dehusking</h2>
      </div>
      ...
      <Modal 
        ...
        title="Stage 6.2 - Dehusking Details"
        size="4xl"
      >
        <div className="max-h-[85vh] overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Cleaning ID</Label>
              <Input 
                type="text"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Brown Rice Output MT</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Rice Husk Generated MT</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Dehusking Efficiency %</Label>
              <Input 
                type="number"
                value={formData.dehuskingEfficiency || ''} 
                onChange={(e) => setFormData({...formData, dehuskingEfficiency: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Rubber Roll ID</Label>
              <Input 
                type="text"
                value={formData.rubberRollId || ''} 
                onChange={(e) => setFormData({...formData, rubberRollId: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
            <div className="space-y-1">
              <Label className="text-sm font-medium text-slate-700">Machine RPM</Label>
              <Input 
                type="number"
                value={formData.machineRpm || ''} 
                onChange={(e) => setFormData({...formData, machineRpm: e.target.value})}
                readOnly={false}
                className={'w-full mt-1.5 bg-white border-slate-300 text-slate-900 focus:ring-blue-500 focus:border-blue-500 rounded-md shadow-sm'}
              />
            </div>
