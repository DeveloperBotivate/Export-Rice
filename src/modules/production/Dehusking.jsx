...
export const Dehusking = () => {
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { dehuskingId: 'DHK-' + Math.floor(Math.random()*10000) };
    
    
    
    
    
    const readOnlyFields = ["cleaningId","productionOrderNo"];
  ...
  const pendingCols = [{"header":"Cleaning ID","accessor":"cleaningId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Cleaned Output (MT)","accessor":"cleanedOutput"},{"header":"Date","accessor":"cleaningDate"},{"header":"Operator","accessor":"cleaningOperator"}];
  const historyCols = [{"header":"Dehusking ID","accessor":"dehuskingId"},{"header":"Cleaning ID","accessor":"cleaningId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Input Qty (MT)","accessor":"dehuskingInputQty"},{"header":"Brown Rice Output (MT)","accessor":"brownRiceOutput"},{"header":"Husk Output (MT)","accessor":"huskOutput"},{"header":"Efficiency %","accessor":"dehuskingEfficiency"},{"header":"Date","accessor":"dehuskingDate"},{"header":"Operator","accessor":"dehuskingOperator"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 2.2 - Dehusking</h2>
      </div>
        ...
        <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                type="number"
                value={formData.dehuskingInputQty || ''} 
                onChange={(e) => setFormData({...formData, dehuskingInputQty: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Brown Rice Output (MT)</Label>
              <Input 
                type="number"
                value={formData.brownRiceOutput || ''} 
                onChange={(e) => setFormData({...formData, brownRiceOutput: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Rice Husk Generated (MT)</Label>
              <Input 
                type="number"
                value={formData.huskOutput || ''} 
                onChange={(e) => setFormData({...formData, huskOutput: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Dehusking Efficiency (%)</Label>
              <Input 
                type="number"
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.dehuskingOperator || ''} 
                onChange={(e) => setFormData({...formData, dehuskingOperator: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                type="datetime-local"
                value={formData.dehuskingDate || ''} 
                onChange={(e) => setFormData({...formData, dehuskingDate: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
