...
export const Separation = () => {
  ...
  const handleActionClick = (item) => {
    ...
    autoFields = { separationId: 'SEP-' + Math.floor(Math.random()*10000) };
    
    
    
    
    const readOnlyFields = ["dehuskingId","productionOrderNo"];
  ...
  const pendingCols = [{"header":"Dehusking ID","accessor":"dehuskingId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Brown Rice Output (MT)","accessor":"brownRiceOutput"},{"header":"Efficiency %","accessor":"dehuskingEfficiency"},{"header":"Date","accessor":"dehuskingDate"},{"header":"Operator","accessor":"dehuskingOperator"}];
  const historyCols = [{"header":"Separation ID","accessor":"separationId"},{"header":"Dehusking ID","accessor":"dehuskingId"},{"header":"Production Order No","accessor":"productionOrderNo"},{"header":"Input Brown Rice (MT)","accessor":"separationInputQty"},{"header":"Separated Brown Rice (MT)","accessor":"separatedBrownRice"},{"header":"Paddy Returned (MT)","accessor":"paddyReturned"},{"header":"Efficiency %","accessor":"separationEfficiency"},{"header":"Date","accessor":"separationDate"},{"header":"Operator","accessor":"separationOperator"}];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-slate-800">Stage 2.3 - Separation</h2>
      </div>
        ...
        <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                type="number"
                value={formData.separationInputQty || ''} 
                onChange={(e) => setFormData({...formData, separationInputQty: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Separated Brown Rice (MT)</Label>
              <Input 
                type="number"
                value={formData.separatedBrownRice || ''} 
                onChange={(e) => setFormData({...formData, separatedBrownRice: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Paddy Returned for Dehusking (MT)</Label>
              <Input 
                type="number"
                value={formData.paddyReturned || ''} 
                onChange={(e) => setFormData({...formData, paddyReturned: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Paddy Separator Machine ID</Label>
              <Input 
                type="text"
                value={formData.separatorMachineId || ''} 
                onChange={(e) => setFormData({...formData, separatorMachineId: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                type="number"
                value={formData.separationEfficiency || ''} 
                onChange={(e) => setFormData({...formData, separationEfficiency: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.separationOperator || ''} 
                onChange={(e) => setFormData({...formData, separationOperator: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
