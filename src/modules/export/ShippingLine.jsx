...
export const ShippingLine = () => {
  ...
  const handleActionClick = (item) => {
    ...
    let autoFields = {};
    if ('ShippingLine' === 'PackingList') autoFields = { packingListNo: 'PL-00' + Math.floor(Math.random()*100) };
    if ('ShippingLine' === 'ContainerBooking') autoFields = { bookingRefNo: 'CB-00' + Math.floor(Math.random()*100) };
    if ('ShippingLine' === 'ContainerLoading') autoFields = { loadingId: 'CL-00' + Math.floor(Math.random()*100) };
    if ('ShippingLine' === 'ShippingBill') autoFields = { shippingBillNo: 'SB-00' + Math.floor(Math.random()*100) };
    if ('ShippingLine' === 'BillOfLading') autoFields = { blNumber: 'BL-00' + Math.floor(Math.random()*100) };
    if ('ShippingLine' === 'CertificateOfOrigin') autoFields = { cooNumber: 'COO-00' + Math.floor(Math.random()*100) };
    if ('ShippingLine' === 'InsuranceAndInvoice') autoFields = { exportInvoiceNo: 'EI-00' + Math.floor(Math.random()*100) };
    if ('ShippingLine' === 'ExportPayment') autoFields = { paymentId: 'EP-00' + Math.floor(Math.random()*100) };
    
    const readOnlyFields = ["loadingId","contractNo","shippingLineName","portOfLoading","portOfDischarge"];
  ...
  const pendingCols = [{"header":"Loading ID","accessor":"loadingId"},{"header":"Booking Ref No","accessor":"bookingRefNo"},{"header":"Contract No","accessor":"contractNo"},{"header":"Container Number","accessor":"containerNumber"},{"header":"Seal Number","accessor":"sealNumber"},{"header":"Net Weight Loaded (MT)","accessor":"netWeightLoaded"},{"header":"Loading Date","accessor":"loadingDate"},{"header":"Surveyor","accessor":"loadingSurveyor"}];
  const historyCols = [{"header":"Loading ID","accessor":"loadingId"},{"header":"Contract No","accessor":"contractNo"},{"header":"Shipping Line Name","accessor":"shippingLineName"},{"header":"Vessel Name","accessor":"vesselName"},{"header":"IMO Number","accessor":"imoNumber"},{"header":"Voyage Number","accessor":"voyageNumber"},{"header":"ETD","accessor":"etd"},{"header":"ETA","accessor":"eta"},{"header":"Port Agent Name","accessor":"portAgentName"},{"header":"VGM Submitted","accessor":"vgmSubmitted"},{"header":"SI Submitted","accessor":"siSubmitted"},{"header":"SI Date","accessor":"siDate"},{"header":"Confirmed Date","accessor":"confirmedDate"}];
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                ...
                onChange={(e) => setFormData({...formData, shippingLineName: e.target.value})}
                readOnly={true}
                className={true ? 'bg-slate-100' : ''}
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.portAgentName || ''} 
                onChange={(e) => setFormData({...formData, portAgentName: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Select 
                ...
                className={false ? 'bg-slate-100' : ''}
              >
                <option value="">Select VGM Submitted (Y/N)</option>
            ...
            <div className="space-y-1.5">
              <Label>SI (Shipping Instruction) Submitted (Y/N)</Label>
              <Select 
                ...
                className={false ? 'bg-slate-100' : ''}
              >
                <option value="">Select SI (Shipping Instruction) Submitted (Y/N)</option>
                <option value="Y">Y</option><option value="N">N</option>
              </Select>
            </div>
            <div className="space-y-1.5">
