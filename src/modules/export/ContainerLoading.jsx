...
export const ContainerLoading = () => {
  ...
  const handleActionClick = (item) => {
    ...
    let autoFields = {};
    if ('ContainerLoading' === 'PackingList') autoFields = { packingListNo: 'PL-00' + Math.floor(Math.random()*100) };
    if ('ContainerLoading' === 'ContainerBooking') autoFields = { bookingRefNo: 'CB-00' + Math.floor(Math.random()*100) };
    if ('ContainerLoading' === 'ContainerLoading') autoFields = { loadingId: 'CL-00' + Math.floor(Math.random()*100) };
    if ('ContainerLoading' === 'ShippingBill') autoFields = { shippingBillNo: 'SB-00' + Math.floor(Math.random()*100) };
    if ('ContainerLoading' === 'BillOfLading') autoFields = { blNumber: 'BL-00' + Math.floor(Math.random()*100) };
    if ('ContainerLoading' === 'CertificateOfOrigin') autoFields = { cooNumber: 'COO-00' + Math.floor(Math.random()*100) };
    if ('ContainerLoading' === 'InsuranceAndInvoice') autoFields = { exportInvoiceNo: 'EI-00' + Math.floor(Math.random()*100) };
    if ('ContainerLoading' === 'ExportPayment') autoFields = { paymentId: 'EP-00' + Math.floor(Math.random()*100) };
        ...
        <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.lotNosLoaded || ''} 
                onChange={(e) => setFormData({...formData, lotNosLoaded: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.batchNosLoaded || ''} 
                onChange={(e) => setFormData({...formData, batchNosLoaded: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Input 
                type="text"
                value={formData.loadingSurveyor || ''} 
                onChange={(e) => setFormData({...formData, loadingSurveyor: e.target.value})}
                readOnly={false}
                className={false ? 'bg-slate-100' : ''}
              />
            </div>
            ...
            <div className="space-y-1.5">
              <Label>Fumigation Done (Y/N)</Label>
              <Select 
                ...
                className={false ? 'bg-slate-100' : ''}
              >
                <option value="">Select Fumigation Done (Y/N)</option>
